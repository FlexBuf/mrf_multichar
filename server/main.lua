...
RegisterNetEvent('mrf_multichar:server:disconnect', function()
    local src = source
    DropPlayer(src, "You have disconnected from QBCore")
end)

RegisterNetEvent('mrf_multichar:server:loadUserData', function(cData)
    local src = source
    if QBCore.Player.Login(src, cData.citizenid) then
        ...
        if Config.LastLoc then
            ...
            TriggerClientEvent('mrf_multichar:client:lastloc', src, coords)
        else
...
RegisterNetEvent('mrf_multichar:server:createCharacter', function(data)
    local src = source
    local newData = {}
    ...
    if QBCore.Player.Login(src, false, newData) then
        if Config.StartingApartment then
            ...
            QBCore.Commands.Refresh(src)
            loadHouseData()
            TriggerClientEvent("mrf_multichar:client:closeNUIdefault", src)
            GiveStarterItems(src)
        end
	end
end)

RegisterNetEvent('mrf_multichar:server:deleteCharacter', function(citizenid)
    local src = source
    QBCore.Player.DeleteCharacter(src, citizenid)
    TriggerClientEvent('QBCore:Notify', src, "You Success Deleted your Charactrer" , "success" )
end)
...
QBCore.Functions.CreateCallback("mrf_multichar:server:GetServerLogs", function(source, cb)
    MySQL.Async.execute('SELECT * FROM server_logs', {}, function(result)
        cb(result)
    end)
end)

QBCore.Functions.CreateCallback("mrf_multichar:server:GetNumberOfCharacters", function(source, cb)
...
QBCore.Functions.CreateCallback("mrf_multichar:server:setupCharacters", function(source, cb)
    local license = QBCore.Functions.GetIdentifier(source, 'license')
    local plyChars = {}
...
print("^2Multicharacter ^7- ^2Made by ^1MARFY^7")
